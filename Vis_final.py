...
opts.defaults(
    opts.Points(alpha=0.8, line_color='black', cmap='Set1',
                height=450, width=600, color='user', size=dim('FixationIndex') * 0.03,
                tools=['hover'], title='Animation',
...
def animate():
    global callback_id
    print(trackData['StimuliName'])
    if button.label == '► Play':
        button.label = '❚❚ Pause'
        callback_id = doc.add_periodic_callback(animate_update, 400)
    else:
        button.label = '► Play'
        callback_id = doc.remove_periodic_callback(callback_id)


start, end = trackData.range('user_index')
...
p6 = layout([[hvplot.state], [animation_slider, button]])
